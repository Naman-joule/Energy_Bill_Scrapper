...
interface CalculationModalProps {
  ...
  editableData: BillData | null;
  applyCalculatedTotals: () => void;
}
...
export const CalculationModal: React.FC<CalculationModalProps> = ({
  ...
  if (!showVerifyModal || !calculatedSummary || !results?.data?.billing_summary) {
    return null;
  }
  ...
  const getScrapedValues = () => {
    ...
    if (editableData?.audit_calculations?.scraped) {
      return editableData.audit_calculations.scraped;
    }
    ...
    const scrapedComponents = results?.data?.bill_components || [];
    ...
    let arrearVal = results?.data?.billing_summary?.total_arrears_lps || 0;
    const finalPayableVal = results?.data?.billing_summary?.net_bill_amount || (netCurrentVal + arrearVal);
    const rebateVal = results?.data?.billing_summary?.rebate || 0;
    const payableTillDueVal = results?.data?.billing_summary?.payable_till_due_date || (finalPayableVal - rebateVal);
